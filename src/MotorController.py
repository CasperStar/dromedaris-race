...
class MotorController():
    def __init__(self, motor_id, pwm_pin, dir_pin_1, dir_pin_2) -> None:
        logging.info("MotorController: Initializing {:02}".format(motor_id))
        ...
        self.async_running_thread = Thread(target = self.__process_async_running_thread, args = ())
        self.async_running_thread.start()



    def start(self, turn_direction, speed):
        logging.info("MotorController: Motor {:02} running {} at {:02}%".format(self.motor_id, turn_direction.name, speed))
        self.set_direction(turn_direction)
        self.pwm_pin.start(speed)

    def stop(self):
        logging.info("MotorController: Motor {:02} stopping".format(self.motor_id))
        self.direction_pin_1.set(False)
        self.direction_pin_2.set(False)
        self.pwm_pin.stop()

    def set_speed(self, speed):
        logging.info("MotorController: Motor {:02} set speed {:02}%".format(self.motor_id, speed))
        self.pwm_pin.set_duty_cycle(speed)

    def set_direction(self, turn_direction):
        logging.info("MotorController: Motor {:02} set direction {}".format(self.motor_id, turn_direction.name))
    ...
    def run_async(self, turn_direction, speed, time_in_ms):
        logging.info("MotorController: Motor {:02} Posting to async queue ({} {}% {}ms)".format(self.motor_id, turn_direction.name, speed, time_in_ms))
